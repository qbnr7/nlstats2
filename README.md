...
**Every "games" count in every report is based on schedule assignment for games that have actually been played, not on whether a flag was thrown.** An official's Games Officiated total, their row in the Officials List and Season Accuracy Ranking, and their Games at Position count in Position Rankings all include every game they were scheduled for in `nlplan/` **that has a matching game file in `data/`**, including games where they had zero recorded penalties at their position. Accuracy percentages are unaffected by this -- those are still only ever calculated from actual graded calls -- but the games/games-at-position counts themselves reflect assignment to a played game, not activity.

A game that's only scheduled in `nlplan/` but has no `.xlsx`/`.xls` game file in `data/` yet does not count toward anyone's totals and gets no per-game report of its own. This is what makes it safe to run the pipeline mid-season, with future weeks already entered in the schedule: those future games are simply invisible to every report until an actual game file for them shows up in `data/`, at which point they start counting like any other game. (A `.csv` file alone doesn't count here -- `03_build_flat_file.py` never reads `.csv` files, so a stray one wouldn't contribute real penalty data.)
